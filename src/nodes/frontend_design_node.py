...
def frontend_design_node(state):

    print('='*20, 'Start frontend_design_node')
    ...
    existing_design = JsonLoader.load("outputs/frontend_design.json")
    existing_content_prompt = ""
    if existing_design:
        existing_content_prompt = f"""
# Existing Frontend Design
기존에 작성된 프론트엔드 설계 내용입니다. 이 내용을 바탕으로 새로운 아키텍처 및 PRD 요구사항을 반영하여 업데이트하세요.
